...
输入一个作品标题或核心观点，系统会自动判断栏目、生成编号、翻译英文标题和子标题；点击生成后调用 LLM 精修，并用 Image2 生成匹配内容的背景。生成后主标题、英文标题、子标题和编号都能直接修改，画布实时同步。也可以一键生成片头和片尾动效，片尾默认引导关注。
...
页面顶部和底部都有反馈入口。用户可以提交报错、流程困惑、生成结果不满意、登录和素材库问题；系统会带上当前模板、栏目、标题和浏览器尺寸，方便后续迭代定位。生产环境优先写入 KV；如果云端反馈箱未配置，会生成 GitHub Issue 草稿。
