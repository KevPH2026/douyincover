...
[English](./README.en.md) | [默认 README](./README.md) | [线上站点](https://dna.superk.ai) | [GitHub](https://github.com/KevPH2026/dna-superk-ai)

Mr.K Cover Studio 不是通用海报工具。它服务的是一种更具体的需求：把一个内容账号的视觉 DNA 固定下来，让每条作品、每个合集、每张主页背景都像从同一套系统里生产出来。

你可以丢一段抖音分享文案、一条作品链接、一张截图、一段片头素材，或者一句自然语言主题。系统会提炼标题、栏目、摘要和背景方向，再用固定的 Mr.K 视觉模板生成封面或 1-2 秒片头动效。
...
| Local Web Studio | 本地打开可视化页面，调图、保存素材、导出 PNG/WebM | `scripts/run_local_studio.sh` |
...
2. 再问任务类型：单条封面、片头动效、解码 DNA、批量换封面、合集封面或主页背景。
3. 再判断分类：优先落在 `AI下半场`、`强者恒强`、`在路上`，不合适再开自定义栏目。
4. 再收素材：单条给链接/文案/截图；片头动效可补图片或视频素材；解码 DNA 至少给 10 条公开作品样本。
5. 最后确认 brief：标题、英文副题、背景方向、比例/时长和编号，确认后生成并保存到素材库。
...
内置 AI 热点拆解、强者金句、一线观察、片头动效、系列合集、主页背景和自定义栏目入口，适合快速开始一张封面，而不是从空白字段填起。
...
未登录用户可以免费使用 3 次 AI 生产能力，包括对话生成封面/片头、解码 DNA 和手动 AI 生图。超过 3 次后会提示输入邮箱验证码登录；邮箱验证通过后自动登录，首次邮箱会自动创建账号。
...
给 Agent 一个抖音主页/作品链接，并补充至少 10 条公开作品样本，例如标题、封面文字、文案或截图转文字。系统会先提炼账号 DNA，再输出栏目、视觉风格和设计方向。
...
- 单条作品封面：`1080x1920`
- 片头覆盖动效：`1080x1920`，`1.2-2 秒`，浏览器导出 WebM
- 合集封面：`1080x1080`
- 主页背景：`1920x640`
